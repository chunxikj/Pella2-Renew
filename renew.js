...
// ── 修复：登录后等待广告链接状态从灰色变为可用 ──────────
// Pella 广告链接在登录后需要 2~3 秒才会从 claimed=true(灰色)
// 切换为 claimed=false(可用)，此处等待 6 秒确保状态稳定。
console.log(’[等待] 登录后等待 6 秒，确保广告链接状态刷新…’);
await delay(6000);
// ────────────────────────────────────────────────────────
...
if (totalUnclaimed === 0) {
console.log(’[等待] 暂未发现可用广告，再等待 5 秒后重新获取…’);
await delay(5000);
servers = await getServers(authData.token);
console.log(`重新获取到 ${servers.length} 个服务器`);
}
// ────────────────────────────────────────────────────────
